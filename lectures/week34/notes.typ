...
As a simple example, consider a coin toss. Here $Omega = {H, T}$ and $cal(E) = {emptyset, Omega, {H}, {T}}$. For a fair coin, we can define a probability with $P(emptyset) = 0$, $P({H}) = P({T}) = 1\/2$, $P(Omega) = 1$.
...
Going back to our example, we select an RV $X: Omega -> RR$ with $X(H) = 1$ and $X(T) = 0$. For any $B in cal(B)(RR)$, we see that
$
  X^(-1)(B) = cases(
    & emptyset & "if" 0 in.not B "and" 1 in.not B,
    { & H} & "if" 0 in.not B "and" 1 in B,
    { & T} & "if" 0 in B "and" 1 in.not B,
...
#solution()[
  ...
  + We also have that strict inequalities are events. This is because
    ...
    $
      ...
      (X >= a) = (X < a)^C,
    $
    meaning these are also events. Any interval and singleton can be made by combining and intersecting sets of these four types. For example,
    $
      (a < X <= b) = (X > a) inter (X <= b),
    $
    meaning $X^(-1)(I) = (X in I)$ is an event whenever $I$ is an interval.

  + First, $RR in cal(M)_X$ because $X^(-1)(RR) = Omega in cal(E)$. If $A in cal(M)_X$, then
    $
      X^(-1)(A^C) = X^(-1)(RR \\ A) = Omega \\ X^(-1)(A) = (X^(-1)(A))^C in cal(E),
    $
    so $A^C in cal(M)_X$. Finally, if $A_1, A_2, ... in cal(M)_X$ then
    $
      X^(-1)(union.big_(n=1)^oo A_n)
      = union.big_(n=1)^oo X^(-1)(A_n)
      in cal(E).
    $
    Therefore $union.big_(n=1)^oo A_n in cal(M)_X$, and $cal(M)_X$ is a $sigma$-algebra on $RR$.

  + Need to show that $X^(-1)(B) in cal(E)$ for every $B in cal(B)(RR)$. Let $cal(I)$ be the family of open intervals in $RR$. Part 2 shows that $cal(I) subset.eq cal(M)_X$. Since the Borel $sigma$-algebra $cal(B)(RR)$ is the smallest $sigma$-algebra containing all open intervals, we have
    $
      cal(B)(RR) subset.eq cal(M)_X.
    $
